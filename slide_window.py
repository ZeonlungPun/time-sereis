...
def predict_sequence_multiple(model,data_x,scalary,window_size,prediction_len):
    ...
    raw_predict=scalary.inverse_transform(np.array(raw_predict))
    # seq_len-1
    cur_frame=data_x[-1]
    new_result=[]
    for i in range(prediction_len):
        pred_result=model.predict(cur_frame[np.newaxis,:,:])[0,0]
        pred_result=scalary.inverse_transform(np.array(pred_result).reshape(1,-1) )
...
pred_seqs=predict_sequence_multiple(cnn_model,xx,scalery,window_size=50,prediction_len=30)
